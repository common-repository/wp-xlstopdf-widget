...
Swiff.Uploader = new Class({
	...
	initialize: function(path, options) {
		...
		//alert(swfpath);
		this.path = path;
		if (!this.path.contains('?')) this.path += '?noCache=' + $time(); // cache in IE
		...
		// target 
		this.target = document.id(this.options.id);
		if (this.target) {
	...
	setOptions: function(options) {
		...
		if (options) { 
			...
			this.parent(options);   // Ovde PUCA NESTO
			if (this.loaded) this.remote('xSetOptions', options);
		}
		//console.log('newoptions: ', this);
		return this;
	}, 
...
Swiff.Uploader.File = new Class({
	...
	setOptions: function(options) {
		if (options) {
			if (options.url) options.url = Swiff.Uploader.qualifyPath(options.url);
			this.base.remote('xFileSetOptions', this.target, options);
